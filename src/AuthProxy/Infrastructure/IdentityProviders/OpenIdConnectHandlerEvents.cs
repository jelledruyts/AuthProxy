...
public class OpenIdConnectHandlerEvents : OpenIdConnectEvents
{
    ...
    public async override Task TokenValidated(TokenValidatedContext context)
    {
        ...
        // TODO: The original token and/or claims should be persisted in the reverse proxy cache,
        // as certain information that isn't sent to the app can still be used within the proxy.
        // For example, the "login_hint" claim (if present) isn't necessarily useful to the app
        // but can be used as the "logout_hint" when a logout is requested towards the IdP.
